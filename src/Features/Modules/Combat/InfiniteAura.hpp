...
public:
    ...
    std::mutex mMutex;
    std::unordered_map<Actor*, uint64_t> mLastHits;
    Actor* mLastTarget = nullptr;
    uint64_t mLastTeleport = 0;
    glm::vec3 mServerPos{ 0, 0, 0 };
    static constexpr float TELEPORT_DELAY = 50.f;  // ms
    void onEnable() override;
    void onDisable() override;
    std::shared_ptr<class MovePlayerPacket> createPacketForPos(glm::vec3 pos);
    std::vector<std::shared_ptr<PlayerAuthInputPacket>> pathToPos(glm::vec3 from, glm::vec3 to);
