...
// Flag indicating whether we have a forced rotation active.
static bool sHasForcedRotation = false;
// When the last rotation update was received.
static std::chrono::steady_clock::time_point sLastForcedRotationUpdate = std::chrono::steady_clock::now();
// Timeout in milliseconds after which we disable forced rotation.
static constexpr int ROTATION_TIMEOUT_MS = 200;
...
void Interface::onRenderEvent(RenderEvent& event)
{
    ...
    // --- Add forced rotation check ---
    auto now = std::chrono::steady_clock::now();
    auto diffMs = std::chrono::duration_cast<std::chrono::milliseconds>(now - sLastForcedRotationUpdate).count();
    if (diffMs > ROTATION_TIMEOUT_MS)
    {
        sHasForcedRotation = false;
    }

    if (!sHasForcedRotation)
    {
        // No forced rotation recently – do not override; let the game handle it.
        return;
    }
    // --- End forced rotation check ---
    ...
    // (Optionally reset usingPaip when appropriate)
...
void Interface::onPacketOutEvent(PacketOutEvent& event)
{
    ...
    if (event.mPacket->getId() == PacketID::PlayerAuthInput && isServerAuthoritative)
    {
        ...
        // Mark that we have forced rotation data and update the timestamp
        sHasForcedRotation = true;
        sLastForcedRotationUpdate = std::chrono::steady_clock::now();
    }
