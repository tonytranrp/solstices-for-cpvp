...
// (Your gamemodesToGetStatsFor and global static variables remain unchanged)
...
// These globals are used for rendering your stats.
...
void SessionInfo::onRenderEvent(RenderEvent& event) {
    ...
    std::string mKillsStr = "Kills: " + std::to_string(kills);
    std::string mDeathsStr = "Deaths: " + std::to_string(deaths);
    std::string mGamesPlayedStr = "Games Played: " + std::to_string(played);

    // NEW: Calculate time played.
    // Assuming NOW returns milliseconds.
    uint64_t now = NOW;
    uint64_t elapsedMillis = now - sessionStart;
    uint64_t elapsedSeconds = elapsedMillis / 1000;
    uint64_t hours = elapsedSeconds / 3600;
    uint64_t minutes = (elapsedSeconds % 3600) / 60;
    uint64_t seconds = elapsedSeconds % 60;

    // Format minutes and seconds with leading zeros if needed.
    std::string mStr = (minutes < 10 ? "0" : "") + std::to_string(minutes);
    std::string sStr = (seconds < 10 ? "0" : "") + std::to_string(seconds);
    std::string mTimePlayedStr = "Time Played: " + std::to_string(hours) + ":" + mStr + ":" + sStr;

    // Use your existing drawing code.
    auto pos = mElement->getPos();
    auto drawList = ImGui::GetForegroundDrawList();
    ImVec2 size = ImVec2(200, 130);
    ...
    ImVec2 titlePos = pos;
    // Adjust title positioning if needed.
    titlePos.x += 64;
    titlePos.y += 3.8;
    ...
    // NEW: Draw the Time Played string below the others.
    ImRenderUtils::drawShadowText(drawList, mTimePlayedStr, ImVec2(pos.x + 10, pos.y + startPadding + 60), ImColor(255, 255, 255, 255), 20, false);
