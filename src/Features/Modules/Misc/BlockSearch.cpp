...
std::unordered_map<BlockPos, HoleEsp::FoundBlock> HoleEsp::mFoundBlocks = {};
void HoleEsp::moveToNext() {
...
void HoleEsp::tryProcessSub(bool& processed, ChunkPos currentChunkPos, int subChunkIndex)
{
    TRY_CALL([&]()
        {
...
bool HoleEsp::processSub(ChunkPos processChunk, int index) {
    if (!ClientInstance::get()->getLevelRenderer()) {
        reset();
        return false;
    }
...
void HoleEsp::onEnable()
{
    gFeatureManager->mDispatcher->listen<RenderEvent, &HoleEsp::onRenderEvent, nes::event_priority::VERY_FIRST>(this);
    gFeatureManager->mDispatcher->listen<BaseTickEvent, &HoleEsp::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->listen<BlockChangedEvent, &HoleEsp::onBlockChangedEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketInEvent, &HoleEsp::onPacketInEvent>(this);
    reset();
}

void HoleEsp::onDisable()
{
    gFeatureManager->mDispatcher->deafen<RenderEvent, &HoleEsp::onRenderEvent>(this);
    gFeatureManager->mDispatcher->deafen<BaseTickEvent, &HoleEsp::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->deafen<BlockChangedEvent, &HoleEsp::onBlockChangedEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketInEvent, &HoleEsp::onPacketInEvent>(this);
    reset();
}

std::vector<int> HoleEsp::getEnabledBlocks()
{
...
void HoleEsp::onBlockChangedEvent(BlockChangedEvent& event)
{
    if (!ClientInstance::get()->getLevelRenderer()) {
        reset();
        return;
...
void HoleEsp::onBaseTickEvent(BaseTickEvent& event)
{
    if (!ClientInstance::get()->getLevelRenderer()) {
        reset();
        return;
...
void HoleEsp::onPacketInEvent(PacketInEvent& event)
{
    if (!ClientInstance::get()->getLevelRenderer()) {
        reset();
        return;
...
void HoleEsp::onRenderEvent(RenderEvent& event)
{
    if (!ClientInstance::get()->getLevelRenderer()) {
        reset();
        return;
