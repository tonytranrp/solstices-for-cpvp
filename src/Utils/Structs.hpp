...
struct AABB {
    ...
    // Constructor: position is the lower corner, size is added to produce the upper corner.
    AABB(const glm::vec3& position, const glm::vec3& size) {
        mMin = position;
        mMax = position + size;
    }

    // Alternate constructor taking explicit min and max.
    AABB(const glm::vec3& min, const glm::vec3& max, bool) : mMin(min), mMax(max) {}
    AABB() : mMin(0), mMax(0) {}

    // Returns the point inside the AABB that is closest to 'point'.
    ...
    // Returns true if this AABB intersects the other.
