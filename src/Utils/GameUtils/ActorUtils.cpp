...
std::shared_ptr<InventoryTransactionPacket> ActorUtils::createAttackTransactionPredictId(Actor* actor, int slot, uint64_t predictedId) {
    auto player = ClientInstance::get()->getLocalPlayer();
    if (!player || !actor) return nullptr;
    ...
    // ✅ Correctly assign **predicted ID**
    cit->mActorId = predictedId;
    cit->mActionType = ItemUseOnActorInventoryTransaction::ActionType::Attack;
    cit->mClickPos = actor->getAABB().getClosestPoint(*player->getPos());
    cit->mPlayerPos = *player->getPos();
